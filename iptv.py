...
def init_chrome_driver():
    ...
    service = Service(executable_path="./chromedriver")  # 替换为实际的 ChromeDriver 路径
    driver = webdriver.Chrome(service=service, options=chrome_options)
    driver.set_page_load_timeout(10)  # 设置页面加载超时为 10 秒
    return driver
...
def validate_urls(urls, num_threads=5):
    ...
    with ThreadPoolExecutor(max_workers=num_threads) as executor:
        ...
        for result in results:
            url, load_time, page_content = result
            if url and load_time:
                valid_urls.append(url)
                # 如果需要保存网页内容，可以在这里处理
                # with open(f"{url.replace('://', '_').replace('/', '_')}.html", "w", encoding="utf-8") as file:
